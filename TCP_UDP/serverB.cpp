...
//        cout << "!!! received: " << buf << endl; //
        ...
        if (buf[0] != '!') {
            ...
            printf("Server B has received a request for Student %d in %s\n", rid, rdname.c_str());
            ...
            if (stu.getId() == INT_MAX) { // if not found
                ...
                printf("Server B has sent \"Student %d not found\" to Main Server\n\n", rid);
                ...
                printf("Server B calculated following academic statistics for Student %d in %s:\n"
                       "Student GPA: %s\n"
                       "Percentage Rank: %s\n"
