...
/**
 * Server B read the file and store the information.
...
int main(void) {
    ...
    // Server B read the file dataA.txt and store the information in map structure "dict".
    unordered_map<string, vector <string> > dict = storeInfo("dataB.txt");
    ...
    if ((rv = getaddrinfo(LOCALHOST, BPORT, &hints, &servinfo)) != 0) {
    ...
    while (1) {
      ...
      if (buf[0] != '!') {
        printf("Server B has received a request for %s\n", buf);
        ...
        printf("Server B found %s distinct students for %s: %s\n", cntstr.c_str(), depart.c_str(), idstr.c_str());
        string result = "B" + idncnt;
